...
const required=[
  ...
  'migrations/0011_project_control_end_to_end.sql','migrations/0012_full_workflow_roles_qc_cco.sql','migrations/0013_field_pr_qc_ati.sql','migrations/0014_qc_continuous_inspection.sql','migrations/0015_public_information_portal.sql','migrations/0016_public_company_portfolio.sql',
  'wrangler.jsonc','package.json','README.md','ROLE-MATRIX.md','ALUR-KENDALI.md','DEPLOY-CHECKLIST.md',
...
for(const m of ['APP-V3.2.1','public_portfolio','public_site_settings','/api/public/site','/api/public/portfolio','publicMediaHandler','servePublicPortal'])if(!worker.includes(m))throw new Error(`Marker public backend hilang: ${m}`);
...
if(pkg.version!=='3.2.1')throw new Error('package version bukan 3.2.1');
...
for(const marker of ['PIC laporan progress wajib Pelaksana Lapangan','PIC Opname wajib QS / Quantity Surveyor','Hanya Head Operational atau Head Unit Bisnis yang dapat memilih vendor PR'])if(!worker.includes(marker))throw new Error(`Workflow guard backend hilang: ${marker}`);
...
console.log('KENDALI V3.2.1 Company Profile & Portfolio preflight OK');
