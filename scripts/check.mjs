...
const required=[
  ...
  'wrangler.jsonc','package.json','README.md','ROLE-MATRIX.md','ALUR-KENDALI.md','DEPLOY-CHECKLIST.md','QA-MATRIX.md','scripts/qa-final.mjs','scripts/worker-role-smoke.mjs',
...
for(const m of ['APP-V3.4.1','public_portfolio','public_site_settings','/api/public/site','/api/public/portfolio','publicMediaHandler','servePublicPortal'])if(!worker.includes(m))throw new Error(`Marker public backend hilang: ${m}`);
...
if(pkg.version!=='3.4.1')throw new Error('package version bukan 3.4.1');
...
for(const marker of ['PIC laporan progress wajib Pelaksana Lapangan','PIC Opname wajib QS / Quantity Surveyor','Hanya Head of Operational atau Head Unit Bisnis yang dapat memilih vendor PR'])if(!worker.includes(marker))throw new Error(`Workflow guard backend hilang: ${marker}`);
...
for(const m of ['qc_reports','ati_reports','workflow_tasks','listWorkflowTasks','syncWorkflowTask','progressWorkflowAction','opnameWorkflowAction','generateDivisionReport','DIVISION_REPORT_SUBMIT','koordinator_engineering','koordinator_supporting','site_manager','pengawas_lapangan'])if(!worker.includes(m))throw new Error(`Marker V3.3.1 backend hilang: ${m}`);
...
console.log('KENDALI V3.4.1 QA Final + Workflow Inbox preflight OK');
