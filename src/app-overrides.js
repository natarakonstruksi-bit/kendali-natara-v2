/**
 * KENDALI App V2.9 — extension layer.
...
(() => {
  ...
  function renderProgress() {
    ...
    const weekly=rs.filter(r=>r.subtype==="mingguan"), daily=rs.filter(r=>r.subtype==="harian"), monthly=rs.filter(r=>r.subtype==="bulanan");
    const pdfs=state.records.filter(r=>r.type==="progress_pdf");
    const latestByProject=new Map();
    weekly.forEach(r=>{
      const prev=latestByProject.get(r.projectId);
      if(!prev || String(r.endDate||"")>String(prev.endDate||"")) latestByProject.set(r.projectId,r);
    });
    return `<div class="op-page-head">
      <div><h2>Laporan Progress</h2><p>Satu pusat laporan lapangan: harian, mingguan, dan bulanan. Laporan mingguan wajib dilengkapi PDF.</p></div>
      <div class="op-actions">
        ${hasRole(ROLES.field)||hasRole(ROLES.super)?`<button class="op-btn primary" data-action="progress-daily">+ Laporan Harian</button>`:""}
        <button class="op-btn" data-action="progress-weekly">+ Laporan Mingguan</button>
        <button class="op-btn" data-action="progress-monthly">+ Laporan Bulanan</button>
        <button class="op-btn" data-action="progress-print">Cetak PDF</button>
        <button class="op-btn" data-action="progress-upload-final">Simpan PDF Client</button>
      </div>
    </div>
    <div class="op-cards">
      <div><b>${daily.length}</b><span>Laporan Harian</span></div>
      <div><b>${weekly.length}</b><span>Laporan Mingguan</span></div>
      <div><b>${monthly.length}</b><span>Laporan Bulanan</span></div>
      <div><b>${num(Math.max(...weekly.map(x=>Number(x.realisasiKumulatif||0)),0))}%</b><span>Realisasi Kumulatif Terakhir</span></div>
    </div>
    <div class="op-table-card">
      <div class="op-card-title">Progress Terakhir per Proyek</div>
      <table class="op-table"><thead><tr><th>Proyek</th><th>Periode</th><th>Rencana Kumulatif</th><th>Realisasi Kumulatif</th><th>Deviasi</th><th>PDF</th></tr></thead><tbody>
      ${[...latestByProject.values()].map(r=>`<tr>
        <td><b>${esc(r.projectName||findProjectName(r.projectId))}</b></td>
        <td>${dateId(r.startDate)} – ${dateId(r.endDate)}</td>
        <td>${num(r.bobotRencanaKumulatif)}%</td>
        <td>${num(r.realisasiKumulatif)}%</td>
        <td class="${Number(r.deviasi)<0?"bad":"good"}">${Number(r.deviasi)>=0?"+":""}${num(r.deviasi)}%</td>
        <td>${r.file?`<a class="op-link" href="${fileUrl(r.file,true)}">Lihat PDF</a>`:"-"}</td>
      </tr>`).join("")||`<tr><td colspan="6" class="empty">Belum ada laporan mingguan.</td></tr>`}
      </tbody></table>
    </div>
    <div class="op-table-card">
      <div class="op-card-title">Riwayat Laporan</div>
      <table class="op-table"><thead><tr><th>Jenis</th><th>Proyek</th><th>Periode/Tanggal</th><th>Rencana</th><th>Realisasi</th><th>Deviasi</th><th>Dokumen</th></tr></thead><tbody>
      ${rs.slice(0,100).map(r=>`<tr><td><b>${esc(r.subtype||"-")}</b></td><td>${esc(r.projectName||findProjectName(r.projectId))}</td><td>${esc(r.subtype==="mingguan"?`${dateId(r.startDate)} – ${dateId(r.endDate)}`:dateId(r.tanggal||r.startDate))}</td><td>${r.bobotRencana!=null?num(r.bobotRencana)+"%":"-"}${r.bobotRencanaKumulatif!=null?` / ${num(r.bobotRencanaKumulatif)}%`:""}</td><td>${r.bobotRealisasi!=null?num(r.bobotRealisasi)+"%":"-"}${r.realisasiKumulatif!=null?` / ${num(r.realisasiKumulatif)}%`:""}</td><td>${r.deviasi!=null?`${Number(r.deviasi)>=0?"+":""}${num(r.deviasi)}%`:"-"}</td><td>${r.file?`<a class="op-link" href="${fileUrl(r.file,true)}">PDF</a>`:"-"}</td></tr>`).join("")||`<tr><td colspan="7" class="empty">Belum ada laporan progress.</td></tr>`}
      </tbody></table>
    </div>
    <div class="op-table-card">
      <div class="op-card-title">PDF Final Tersimpan untuk Client</div>
      <table class="op-table"><thead><tr><th>Jenis</th><th>Proyek</th><th>Periode</th><th>File</th><th>Oleh</th></tr></thead><tbody>
      ${pdfs.slice(0,50).map(r=>`<tr><td>${esc(r.subtype)}</td><td>${esc(r.projectName)}</td><td>${dateId(r.periode)}</td><td><a class="op-link" href="${fileUrl(r.file,true)}">${esc(r.file?.name||"PDF")}</a></td><td>${esc(r.oleh||"-")}</td></tr>`).join("")||`<tr><td colspan="5" class="empty">Belum ada PDF final.</td></tr>`}
      </tbody></table>
    </div>`;
  }
  ...
  function progressForm(subtype) {
    const weekly=subtype==="mingguan";
    const title=weekly?"Laporan Progress Mingguan":"Laporan Progress Bulanan";
    const body=`<form id="progress-form">
      <div class="op-note">Bobot diisi manual berdasarkan laporan progress yang dikirim. Tidak mengambil RAB.</div>
      <div class="op-grid two">
        ${field("Proyek",selectProject())}
        ${weekly?field("Minggu ke-",input("mingguKe","number","","min=1 required")):field("Tanggal laporan",input("tanggal","date",today(),"required"))}
        ${field("Mulai tanggal",input("startDate","date",today(),"required"))}
        ${field("Sampai tanggal",input("endDate","date",today(),"required"))}
        ${field("Bobot rencana (%)",input("bobotRencana","number","0","step=0.01 min=0 max=100"))}
        ${field("Bobot rencana kumulatif (%)",input("bobotRencanaKumulatif","number","0","step=0.01 min=0 max=100"))}
        ${field("Bobot realisasi (%)",input("bobotRealisasi","number","0","step=0.01 min=0 max=100"))}
        ${field("Bobot realisasi kumulatif (%)",input("realisasiKumulatif","number","0","step=0.01 min=0 max=100"))}
        ${field("Deviasi (%)",input("deviasi","number","0","step=0.01"))}
      </div>
      ${weekly?field("PDF Laporan Mingguan (wajib)",`<input name="file" type="file" accept="application/pdf" required>`):""}
      ${field("Ringkasan",textarea("ringkasan","","Ringkasan berdasarkan laporan yang dikirim"))}
    </form>`;
    modal(title,body,async()=>{});
    const m=document.getElementById("op-modal");
    m.querySelector("[data-save]").onclick=async()=>{
      const form=m.querySelector("#progress-form"),fd=new FormData(form);
      const p=state.projects.find(x=>x.id===fd.get("projectId"));
      if(!p)throw new Error("Proyek wajib dipilih");
      if(weekly && !form.querySelector('[name="file"]')?.files?.[0])throw new Error("PDF laporan mingguan wajib di-upload");
      const vals=["bobotRencana","bobotRencanaKumulatif","bobotRealisasi","realisasiKumulatif","deviasi"].map(k=>Number(fd.get(k)||0));
      if(vals.slice(0,4).some(v=>v<0||v>100))throw new Error("Bobot harus 0–100%");
      let file=null;
      if(weekly)file=await uploadFile(form.querySelector('[name="file"]').files[0],"laporan-progress-mingguan");
      const rec={
        id:uid("PROG"),type:"progress",subtype,
        projectId:p.id,projectName:projectName(p),oleh:state.user?.username||state.user?.email||"",
        tanggal:fd.get("tanggal")||fd.get("endDate"),startDate:fd.get("startDate"),endDate:fd.get("endDate"),
        mingguKe:fd.get("mingguKe")||null,bobotRencana:vals[0],bobotRencanaKumulatif:vals[1],
        bobotRealisasi:vals[2],realisasiKumulatif:vals[3],deviasi:vals[4],
        ringkasan:fd.get("ringkasan")||"",file,status:"Tersimpan",createdAt:new Date().toISOString()
      };
      await apiUpsert("operational_requests",rec.id,rec);
      const pd={...(p.data||{}),aktual:rec.realisasiKumulatif,progressTerakhir:{sumber:"Laporan Progress",subtype,tanggal:rec.endDate||rec.tanggal,actual:rec.realisasiKumulatif,deviasi:rec.deviasi,reportId:rec.id}};
      await apiUpsert("projects",p.id,pd);
      state.records.unshift(rec);Object.assign(p,{data:pd});m.remove();
      toast(`Laporan ${weekly?"mingguan":"bulanan"} tersimpan dan progress proyek diperbarui`);layout();
    };
  }
  ...
  /* ------------------------- Vendor -------------------------------- */
  function vendorActions(r){
    const b=(a,l,c="")=>`<button class="op-mini ${c}" data-vendor-action="${a}" data-id="${esc(r.id)}">${esc(l)}</button>`;
    let s="";
    if(hasRole(ROLES.super) && r.status==="Menunggu Persetujuan Project Manager / Superintendent")s+=b("approve-pm","ACC PM / Superintendent","primary");
    if(hasRole(ROLES.headOps) && r.status==="Menunggu Head Operational")s+=b("headops","ACC Head Operational","primary");
    if(hasRole(ROLES.headBU) && r.status==="Menunggu Head BU / Manager Natara")s+=b("headbu","Approve & Pilih Vendor","primary");
    if(hasRole(ROLES.adminTeknik) && r.status==="Menunggu Admin Teknik")s+=b("admintek","Terbitkan Surat","primary");
    if(hasRole(ROLES.super) && r.status==="Menunggu PM Upload Surat")s+=b("upload-surat","Upload Surat","primary");
    return s||"-";
  }

  function renderVendor(){
    const rs=state.records.filter(r=>r.type==="vendor_request").sort((a,b)=>String(b.createdAt||"").localeCompare(String(a.createdAt||"")));
    return `<div class="op-page-head"><div><h2>Pengajuan Pekerjaan Vendor</h2><p>Pelaksana → PM/Superintendent → Head Operational → Manager Natara → Admin Teknik → PM. Nomor FR-pR diterbitkan setelah approval PM.</p></div>
      ${hasRole(ROLES.field)||hasRole(ROLES.super)?`<button class="op-btn primary" data-action="vendor-add">+ Pengajuan Pekerjaan Vendor</button>`:""}
    </div>
    <div class="op-table-card"><table class="op-table"><thead><tr><th>No.</th><th>Tanggal</th><th>Proyek</th><th>Pekerjaan</th><th>Total HPP</th><th>Vendor</th><th>Vendor Terpilih</th><th>Status</th><th>Aksi</th></tr></thead><tbody>
    ${rs.map(r=>`<tr><td>${esc(r.nomor||"Menunggu ACC PM")}</td><td>${dateId(r.tanggal)}</td><td>${esc(r.projectName)}</td><td>${(r.items||[]).length} item</td><td>${money(r.totalHpp)}</td><td>${esc((r.vendors||[]).map(v=>v.nama).join(", "))}</td><td>${esc(r.vendorTerpilih?.nama||"-")}</td><td><span class="badge ${String(r.status).includes("Ditolak")?"bad":String(r.status).includes("Menunggu")?"warn":"good"}">${esc(r.status)}</span></td><td>${vendorActions(r)}</td></tr>`).join("")||`<tr><td colspan="9" class="empty">Belum ada pengajuan pekerjaan vendor.</td></tr>`}
    </tbody></table></div>`;
  }

  function vendorForm(){
    const body=`<form id="vendor-form">
      <div class="op-note">Nomor FR-pR dibuat otomatis setelah pengajuan mendapat ACC PM/Superintendent. Masukkan minimal satu vendor; bila ATI belum memiliki vendor, kandidat dapat ditambahkan oleh tim lapangan.</div>
      <div class="op-grid two">${field("Proyek",selectProject())}${field("Tanggal",input("tanggal","date",today(),"required"))}${field("Durasi pekerjaan",input("durasi","text","","required"))}${field("Lokasi",input("lokasi","text","","required"))}${field("Kode proyek",input("kodeProyek","text",""))}</div>
      <div class="op-section-label">Daftar Pekerjaan / Spesifikasi Material atau Jasa</div>
      <div id="vendor-items" class="op-repeat"></div>
      <button type="button" class="op-btn" data-add-vitem>+ Tambah Pekerjaan</button>
      <div class="op-section-label">Pilihan Vendor</div>
      <div id="vendor-options" class="op-repeat"></div>
      <button type="button" class="op-btn" data-add-vendor>+ Tambah Vendor</button>
      ${field("Catatan",textarea("catatan","","Keterangan/ketentuan pekerjaan"))}
    </form>`;
    modal("Pengajuan Pekerjaan Vendor",body,async()=>{});
    const m=document.getElementById("op-modal"),items=m.querySelector("#vendor-items"),vendors=m.querySelector("#vendor-options");
    const addItem=()=>{
      const d=document.createElement("div");d.className="op-repeat-row vendor-item";
      d.innerHTML=`${field("Pekerjaan",input("pekerjaan","text","","required"))}${field("Spesifikasi material/jasa",input("spesifikasi","text","","required"))}${field("Volume",input("volume","number","0","step=0.01 min=0"))}${field("Satuan",input("satuan","text","unit","required"))}${field("HPP/unit",input("hpp","number","0","min=0"))}<button type="button" class="op-mini danger">×</button>`;
      d.querySelector(".danger").onclick=()=>d.remove();items.appendChild(d);
    };
    const addVendor=()=>{
      const d=document.createElement("div");d.className="op-repeat-row vendor-option";
      d.innerHTML=`${field("Nama vendor",input("vendorNama","text","","required"))}${field("Vendor ID",input("vendorId","text",""))}${field("Harga penawaran",input("vendorHarga","number","0","min=0"))}${field("Term pembayaran",input("vendorTerm","text",""))}${field("Lead time",input("vendorLead","text",""))}${field("Rincian harga",textarea("vendorDetail","","Rincian harga vendor per item"))}<button type="button" class="op-mini danger">×</button>`;
      d.querySelector(".danger").onclick=()=>d.remove();vendors.appendChild(d);
    };
    addItem();addVendor();
    m.querySelector("[data-add-vitem]").onclick=addItem;m.querySelector("[data-add-vendor]").onclick=addVendor;
    m.querySelector("[data-save]").onclick=async()=>{
      const form=m.querySelector("#vendor-form"),fd=new FormData(form),p=state.projects.find(x=>x.id===fd.get("projectId"));
      if(!p)throw new Error("Proyek wajib dipilih");
      const its=[...items.querySelectorAll(".vendor-item")].map(x=>({pekerjaan:x.querySelector('[name="pekerjaan"]').value.trim(),spesifikasi:x.querySelector('[name="spesifikasi"]').value.trim(),volume:Number(x.querySelector('[name="volume"]').value||0),satuan:x.querySelector('[name="satuan"]').value,hppUnit:Number(x.querySelector('[name="hpp"]').value||0)}));
      if(!its.length||its.some(x=>!x.pekerjaan||!x.satuan))throw new Error("Minimal satu pekerjaan dan satuan wajib diisi");
      its.forEach(x=>x.totalHpp=x.volume*x.hppUnit);
      const vs=[...vendors.querySelectorAll(".vendor-option")].map(x=>({nama:x.querySelector('[name="vendorNama"]').value.trim(),vendorId:x.querySelector('[name="vendorId"]').value.trim(),harga:Number(x.querySelector('[name="vendorHarga"]').value||0),term:x.querySelector('[name="vendorTerm"]').value,leadTime:x.querySelector('[name="vendorLead"]').value,detail:x.querySelector('[name="vendorDetail"]').value})).filter(x=>x.nama);
      if(!vs.length)throw new Error("Minimal satu kandidat vendor wajib diisi");
      const rec={id:uid("VDR"),type:"vendor_request",nomor:null,tanggal:fd.get("tanggal"),projectId:p.id,projectName:projectName(p),kodeProyek:fd.get("kodeProyek"),durasi:fd.get("durasi"),lokasi:fd.get("lokasi"),items:its,totalHpp:its.reduce((s,x)=>s+x.totalHpp,0),vendors:vs,catatan:fd.get("catatan"),pemohon:state.user?.username||"",status:"Menunggu Persetujuan Project Manager / Superintendent",createdAt:new Date().toISOString(),history:[{status:"Diajukan",oleh:state.user?.username||"",tanggal:new Date().toISOString()}]};
      await apiUpsert("operational_requests",rec.id,rec);state.records.unshift(rec);m.remove();toast("Pengajuan vendor dikirim ke Project Manager / Superintendent");layout();
    };
  }

  function nextVendorNumber(){
    const approved=state.records.filter(x=>x.type==="vendor_request"&&x.nomor&&x.nomor.match(/^\d{3}\/FR-pR\/\d{2}\/\d{4}$/));
    const max=approved.reduce((n,x)=>Math.max(n,Number(String(x.nomor).split("/")[0])||0),0);
    const d=new Date();return `${String(max+1).padStart(3,"0")}/FR-pR/${String(d.getMonth()+1).padStart(2,"0")}/${d.getFullYear()}`;
  }

  async function vendorTransition(id,action){
    const r=state.records.find(x=>x.id===id);if(!r)return;
    let next=r.status;
    if(action==="approve-pm"){r.nomor=r.nomor||nextVendorNumber();next="Menunggu Head Operational";}
    if(action==="headops")next="Menunggu Head BU / Manager Natara";
    if(action==="headbu"){
      const names=(r.vendors||[]).map((v,i)=>`${i+1}. ${v.nama} — ${money(v.harga)} — ${v.term||"-"} — ${v.leadTime||"-"}`).join("\n");
      const chosen=prompt(`Pilih vendor yang disetujui:\n${names}`,"1");
      if(chosen===null)return;
      const idx=Number(chosen)-1;if(!r.vendors[idx])throw new Error("Pilihan vendor tidak valid");
      r.vendorTerpilih=r.vendors[idx];next="Menunggu Admin Teknik";
    }
    if(action==="admintek"){r.surat={status:"Surat dibuat Admin Teknik",dibuatAt:new Date().toISOString()};next="Menunggu PM Upload Surat";}
    if(action==="upload-surat"){
      ...
      modal("Upload Surat Pekerjaan Vendor",body,async()=>{});
      const m=document.getElementById("op-modal");
      m.querySelector("[data-save]").onclick=async()=>{
        const f=m.querySelector("#vendor-upload-form"),fi=f.querySelector('[name="file"]');
        if(!fi.files[0])throw new Error("File surat wajib dipilih");
        const file=await uploadFile(fi.files[0],"surat-vendor");
        r.surat={...r.surat,file,catatan:f.querySelector('[name="catatan"]').value,uploadedAt:new Date().toISOString()};
        r.status="Selesai · Surat terarsip";r.history=[...(r.history||[]),{status:r.status,oleh:state.user?.username||"",tanggal:new Date().toISOString()}];
        await apiUpsert("operational_requests",r.id,r);m.remove();toast("Surat vendor tersimpan dan terarsip berdasarkan proyek");layout();
      };
      return;
    }
    r.status=next;r.history=[...(r.history||[]),{status:next,oleh:state.user?.username||"",tanggal:new Date().toISOString(),catatan:action}];
    await apiUpsert("operational_requests",r.id,r);toast(`Vendor → ${next}`);layout();
  }
  ...
  function bindTab(){
    ...
    root.querySelectorAll("[data-action]").forEach(b=>b.onclick=()=>{
      ...
      else if(a==="progress-print"){
        const rs=state.records.filter(r=>r.type==="progress");
        if(!rs.length) return toast("Belum ada laporan progress",false);
        printReports(rs,"Laporan Progress KENDALI");
      } else if(a==="progress-upload-final")uploadFinalReportForm();
    });
...
`;
  ...
  function addNav(){
    ...
    if(existing)return;
    const anchor=
      candidates.find(x=>(x.textContent||"").trim()==="Kerja Saya") ||
      candidates.find(x=>(x.textContent||"").trim()==="Tugas Saya") ||
      candidates.find(x=>(x.textContent||"").trim()==="Proyek") ||
      candidates.find(x=>(x.textContent||"").trim()==="Dashboard");
    if(!anchor)return;
    const nav=anchor.cloneNode(true);
    nav.textContent="⚙  Departemen Operasional";
    nav.removeAttribute("href");
    nav.style.cursor="pointer";
    nav.dataset.kendaliOperationalNav="1";
    nav.onclick=()=>{location.hash="#/operasional";};
