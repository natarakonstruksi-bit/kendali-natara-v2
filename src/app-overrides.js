...
(() => {
  ...
  function dailyForm(){
    ...
    const m=document.getElementById("op-modal");m.querySelector("[data-save]").onclick=async()=>{
      ...
      let file=null; const fi=m.querySelector('[name="file"]'); if(fi?.files?.[0]) file=await uploadFile(fi.files[0],"laporan-progress"); const rec={id:uid("HARI"),type:"progress",subtype:"harian",projectId:p.id,projectName:projectName(p),oleh:state.user?.username||"",tanggal:fd.get("tanggal"),progress:Number(fd.get("progress")||0),cuaca:fd.get("cuaca"),pekerjaan:fd.get("pekerjaan"),tenaga:fd.get("tenaga"),kendala:fd.get("kendala"),catatan:fd.get("catatan"),file,status:"Tersimpan",createdAt:new Date().toISOString()};
      await apiUpsert("operational_requests",rec.id,rec);
      const pd={...(p.data||{}),aktual:rec.progress,progressTerakhir:{sumber:"Laporan Progress Harian",tanggal:rec.tanggal,actual:rec.progress,reportId:rec.id}};
      await apiUpsert("projects",p.id,pd); Object.assign(p,{data:pd});
      state.records.unshift(rec);m.remove();toast("Laporan harian tersimpan dan progres proyek diperbarui");layout();
    };
  }
  ...
  function ccoForm(){const body=`<form id="cco-form"><div class="op-grid two">${field("Proyek",selectProject())}${field("Judul CCO",input("judul","text","","required"))}</div>${field("Alasan / latar belakang",textarea("alasan","","Kenapa item perlu tambah/kurang"))}<div class="op-section-label">Item perubahan</div><div id="cco-rows" class="op-repeat"></div><button type="button" class="op-btn" data-add-cco>+ Tambah Item</button>${field("Lampiran foto / dokumen",`<input type="file" name="file" accept="image/*,.pdf,.dwg,.xlsx,.docx" multiple>`)}${field("Catatan",textarea("catatan","","Informasi penting untuk Admin Teknik"))}</form>`;modal("Ajukan CCO / Pekerjaan Tambah-Kurang",body,async()=>{});const m=document.getElementById("op-modal"),rows=m.querySelector("#cco-rows");const add=()=>{const d=document.createElement("div");d.className="op-repeat-row cco-row";d.innerHTML=`${field("Jenis",`<select name="jenis"><option>Tambah</option><option>Kurang</option></select>`)}${field("Item",input("item","text","","required"))}${field("Volume",input("volume","number","0","step=0.01 min=0"))}${field("Satuan",input("satuan","text","unit"))}${field("Spesifikasi",input("spesifikasi","text",""))}<button type="button" class="op-mini danger">×</button>`;d.querySelector(".danger").onclick=()=>d.remove();rows.appendChild(d);};add();m.querySelector("[data-add-cco]").onclick=add;m.querySelector("[data-save]").onclick=async()=>{const fd=new FormData(m.querySelector("form")),p=state.projects.find(x=>x.id===fd.get("projectId"));if(!p)throw new Error("Proyek wajib dipilih");const its=[...rows.querySelectorAll(".cco-row")].map(row=>({jenis:row.querySelector('[name="jenis"]').value,item:row.querySelector('[name="item"]').value,volume:Number(row.querySelector('[name="volume"]').value||0),satuan:row.querySelector('[name="satuan"]').value,spesifikasi:row.querySelector('[name="spesifikasi"]').value}));if(!its.length)throw new Error("Minimal satu item CCO");let files=[];const fi=m.querySelector('[name="file"]');if(fi?.files?.length){for(const f of fi.files)files.push(await uploadFile(f,"cco"));}const rec={id:uid("CCO"),type:"cco",projectId:p.id,projectName:projectName(p),judul:fd.get("judul"),alasan:fd.get("alasan"),items:its,files,catatan:fd.get("catatan"),status:"Menunggu Persetujuan Superintendent/PM",oleh:state.user?.username||"",createdAt:new Date().toISOString(),history:[{status:"Diajukan",oleh:state.user?.username||"",tanggal:new Date().toISOString()}]};await apiUpsert("operational_requests",rec.id,rec);state.records.unshift(rec);m.remove();toast("CCO dikirim ke Superintendent / Project Manager");layout();};}
  async function ccoTransition(id,action){
    const r=state.records.find(x=>x.id===id); if(!r)return;
    let next=r.status, extra={};
    if(action==="approve-super") next="Menunggu Admin Teknik";
    if(action==="send-client") next="Menunggu Persetujuan Client";
    if(action==="client-approve") next="Menunggu QS RAB";
    if(action==="client-reject-price"){
      const reason=prompt("Alasan client meminta hitung ulang harga:","");
      if(reason===null)return; r.alasanClient=reason; next="Kembali ke QS untuk Hitung Ulang";
    }
    if(action==="client-reject-no"){
      const reason=prompt("Catatan client tidak melanjutkan CCO:","");
      if(reason===null)return; r.alasanClient=reason; next="Tidak Dilanjutkan CCO";
    }
    if(action==="client-rab-approve") next="Menunggu Diteruskan ke Lapangan";
    if(action==="send-field") next="Diteruskan ke Lapangan";
    if(action==="done") next="Dikerjakan / CCO Selesai";
    if(action==="qs-rab"){
      const body=`<form id="qs-rab-form">${field("Nilai RAB Addendum",input("rabAddendum","number",String(r.rabAddendum||0),"min=0 step=1 required"))}${field("File RAB Addendum (PDF)",`<input name="file" type="file" accept="application/pdf" required>`)}${field("Catatan QS",textarea("catatan",r.catatanQS||"","Catatan perhitungan / revisi"))}</form>`;
      modal("RAB Addendum oleh QS",body,async()=>{});
      const m=document.getElementById("op-modal");
      m.querySelector("[data-save]").onclick=async()=>{
        try{
          const f=m.querySelector("#qs-rab-form"),fd=new FormData(f),fi=f.querySelector('[name="file"]');
          if(!fi.files[0])throw new Error("File RAB Addendum wajib diunggah.");
          const file=await uploadFile(fi.files[0],"cco-rab");
          r.rabAddendum=Number(fd.get("rabAddendum")||0); r.rabFile=file; r.catatanQS=fd.get("catatan")||"";
          r.status="Menunggu Persetujuan Client RAB";
          r.history=[...(r.history||[]),{status:r.status,oleh:state.user?.username||"",tanggal:new Date().toISOString(),catatan:"RAB Addendum dikirim QS"}];
          await apiUpsert("operational_requests",r.id,r); state.records=state.records.map(x=>x.id===r.id?r:x);
          m.remove(); toast("RAB Addendum tersimpan dan dikirim ke Client"); layout();
        }catch(e){toast(e.message||"Gagal menyimpan RAB",false);}
      };
      return;
    }
    r.status=next; Object.assign(r,extra);
    r.history=[...(r.history||[]),{status:next,oleh:state.user?.username||"",tanggal:new Date().toISOString(),catatan:action}];
    await apiUpsert("operational_requests",r.id,r); toast(`CCO → ${next}`); layout();
  }

  /* ------------------------- Vendor -------------------------------- */
  function renderVendor(){const rs=state.records.filter(r=>r.type==="vendor_request").sort((a,b)=>String(b.createdAt||"").localeCompare(String(a.createdAt||"")));return `<div class="op-page-head"><div><h2>Pekerjaan Vendor</h2><p>Pengajuan → Head Operational → Head BU/Manager Natara → Admin Teknik → PM upload surat → arsip proyek.</p></div>${hasRole(ROLES.field)||hasRole(ROLES.super)?`<button class="op-btn primary" data-action="vendor-add">+ Ajukan Pekerjaan Vendor</button>`:""}</div><div class="op-table-card"><table class="op-table"><thead><tr><th>Nomor</th><th>Tanggal</th><th>Proyek</th><th>Durasi</th><th>Total HPP</th><th>Vendor</th><th>Status</th><th>Aksi</th></tr></thead><tbody>${rs.map(r=>`<tr><td>${esc(r.nomor||"Menunggu ACC PM")}</td><td>${dateId(r.tanggal)}</td><td>${esc(r.projectName)}</td><td>${esc(r.durasi)}</td><td>${money(r.totalHpp)}</td><td>${esc((r.vendors||[]).map(v=>v.nama).join(", "))}</td><td>${esc(r.status)}</td><td>${vendorActions(r)}</td></tr>`).join("")||`<tr><td colspan="8" class="empty">Belum ada pengajuan vendor.</td></tr>`}</tbody></table></div>`;}
  function vendorActions(r){const b=(a,l,c="")=>`<button class="op-mini ${c}" data-vendor-action="${a}" data-id="${esc(r.id)}">${esc(l)}</button>`;let s="";if(hasRole(ROLES.super)&&r.status==="Menunggu Persetujuan PM/Superintendent")s+=b("pm-approve","ACC PM/Superintendent","primary");if(hasRole(ROLES.headOps)&&r.status==="Menunggu Head Operational")s+=b("headops","ACC Head Ops","primary");if(hasRole(ROLES.headBU)&&r.status==="Menunggu Head BU / Manager Natara")s+=b("headbu","APPROVE HBU","primary");if(hasRole(ROLES.adminTeknik)&&r.status==="Menunggu Admin Teknik")s+=b("admintek","Buat Surat");if(hasRole(ROLES.super)&&r.status==="Menunggu PM Upload Surat")s+=b("upload-surat","Upload Surat");return s||"-";}
  function vendorForm(){const body=`<form id="vendor-form"><div class="op-grid two">${field("Proyek",selectProject())}${field("Tanggal",input("tanggal","date",today(),"required"))}${field("Kode proyek",input("kodeProyek","text",""))}${field("Durasi pekerjaan",input("durasi","text","","required"))}${field("Lokasi",input("lokasi","text","","required"))}</div><div class="op-section-label">Daftar pekerjaan / HPP</div><div id="vendor-items" class="op-repeat"></div><button type="button" class="op-btn" data-add-vitem>+ Tambah Pekerjaan</button><div class="op-section-label">Pilihan vendor</div><div id="vendor-options" class="op-repeat"></div><button type="button" class="op-btn" data-add-vendor>+ Tambah Vendor</button>${field("Catatan",textarea("catatan","","Spesifikasi/ketentuan tambahan"))}</form>`;modal("Pengajuan Pekerjaan Vendor",body,async()=>{});const m=document.getElementById("op-modal"),items=m.querySelector("#vendor-items"),vendors=m.querySelector("#vendor-options");const addItem=()=>{const d=document.createElement("div");d.className="op-repeat-row vendor-item";d.innerHTML=`${field("Pekerjaan / spesifikasi",input("pekerjaan","text","","required"))}${field("Material/Jasa",input("spesifikasi","text",""))}${field("Volume",input("volume","number","0","step=0.01"))}${field("Satuan",input("satuan","text","unit"))}${field("HPP/unit",input("hpp","number","0","min=0"))}<button type="button" class="op-mini danger">×</button>`;d.querySelector(".danger").onclick=()=>d.remove();items.appendChild(d);};const addVendor=()=>{const d=document.createElement("div");d.className="op-repeat-row vendor-option";d.innerHTML=`${field("Nama vendor",input("vendorNama","text","","required"))}${field("Harga vendor",input("vendorHarga","number","0","min=0"))}${field("Rincian harga",textarea("vendorDetail","","Detail penawaran vendor"))}<button type="button" class="op-mini danger">×</button>`;d.querySelector(".danger").onclick=()=>d.remove();vendors.appendChild(d);};addItem();addVendor();m.querySelector("[data-add-vitem]").onclick=addItem;m.querySelector("[data-add-vendor]").onclick=addVendor;m.querySelector("[data-save]").onclick=async()=>{const fd=new FormData(m.querySelector("form")),p=state.projects.find(x=>x.id===fd.get("projectId"));if(!p)throw new Error("Proyek wajib dipilih");const its=[...items.querySelectorAll(".vendor-item")].map(x=>({pekerjaan:x.querySelector('[name="pekerjaan"]').value,spesifikasi:x.querySelector('[name="spesifikasi"]').value,volume:Number(x.querySelector('[name="volume"]').value||0),satuan:x.querySelector('[name="satuan"]').value,hppUnit:Number(x.querySelector('[name="hpp"]').value||0)}));its.forEach(x=>x.totalHpp=x.volume*x.hppUnit);const vs=[...vendors.querySelectorAll(".vendor-option")].map(x=>({nama:x.querySelector('[name="vendorNama"]').value,harga:Number(x.querySelector('[name="vendorHarga"]').value||0),detail:x.querySelector('[name="vendorDetail"]').value}));const nomor="";const rec={id:uid("VDR"),type:"vendor_request",nomor,tanggal:fd.get("tanggal"),projectId:p.id,projectName:projectName(p),kodeProyek:fd.get("kodeProyek"),durasi:fd.get("durasi"),lokasi:fd.get("lokasi"),items:its,totalHpp:its.reduce((s,x)=>s+x.totalHpp,0),vendors:vs,catatan:fd.get("catatan"),pemohon:state.user?.username||"",status:"Menunggu Persetujuan PM/Superintendent",createdAt:new Date().toISOString(),history:[{status:"Diajukan",oleh:state.user?.username||"",tanggal:new Date().toISOString()}]};await apiUpsert("operational_requests",rec.id,rec);state.records.unshift(rec);m.remove();toast("Pengajuan pekerjaan vendor dikirim ke Project Manager / Superintendent");layout();};}
  async function vendorTransition(id,action){
    const r=state.records.find(x=>x.id===id);if(!r)return;let next=r.status;
    if(action==="pm-approve"){ const approvedIds=state.records.filter(x=>x.type==="vendor_request"&&x.vendorTerpilih&&String(x.nomor||"").match(/^\d{3}\/FR-pR\/\d{2}\/\d{4}$/)); const {m:mo,y}=monthYear(); const nums=approvedIds.filter(x=>String(x.nomor).includes(`/FR-pR/${mo}/${y}`)).map(x=>Number(String(x.nomor).split("/")[0])).filter(Number.isFinite); const nextNo=(nums.length?Math.max(...nums):0)+1; r.nomor=`${String(nextNo).padStart(3,"0")}/FR-pR/${mo}/${y}`; next="Menunggu Head Operational"; }
  ...
  function bindTab(){
    ...
    root.querySelectorAll("[data-logistik-action]").forEach(b=>b.onclick=async()=>{const r=state.records.find(x=>x.id===b.dataset.id);if(!r)return;r.status="Diproses Admin Logistik";r.processedBy=state.user?.username||"";r.processedAt=new Date().toISOString();r.history=[...(r.history||[]),{status:r.status,oleh:state.user?.username||"",tanggal:new Date().toISOString()}];await apiUpsert("operational_requests",r.id,r);toast("Pengajuan diterima Admin Logistik");layout();});
